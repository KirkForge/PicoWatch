...
The Shogun platform defends software supply chains across multiple layers. PicoSentry covers static scanning (L2), IronDome covers runtime sandboxing and behavioral analysis (L3/L4), and 55NDeep provides deterministic verification. No product addresses the emerging threat surface of LLM interactions: prompt injection, output manipulation, and lack of observability.

PicoWatch must fill this gap as a standalone tool that also integrates into Shogun's Iron Dome firewall when available.
...
3. **Three independent subsystems**: PromptGuard, OutputGuard, and TelemetrySink are separate modules that can be used independently. Shogun integration composes them into the firewall pipeline.
...
✅ Positive: Standalone operation — no Shogun dependency required.
