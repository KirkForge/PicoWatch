...
The Shogun platform already uses PicoSentry for static scanning and IronDome for runtime sandboxing. PicoWatch should follow the same hardening model.
...
4. **Self-sandbox**: CI runs IronDome sandbox on any post-install hooks.
...
5. **Rate limiting**: Built-in per-IP rate limiting on HTTP daemon (same model as Shogun).
...
3. **Audit log integrity**: SQLite audit log uses WAL mode with checksums (same model as Shogun's WORM audit).

### Build & Release

1. **Reproducible builds**: `pip install picowatch==X.Y.Z` produces the same wheel hash across environments.
2. **SLSA Level 3**: Build provenance attestation on every release, matching PicoSentry/IronDome.
...
✅ Positive: SLSA L3 + SBOM matches PicoSentry/IronDome hardening.
